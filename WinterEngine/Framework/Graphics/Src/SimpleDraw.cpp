...
using namespace WinterEngine::Math::Constants;
...
namespace
{
	...
	void SimpleDrawImpl::Render(const Camera& camera)
	{
		...
		mMeshBuffer.SetTopology(MeshBuffer::Topology::Line);
		mMeshBuffer.Render();
...
void SimpleDraw::AddSphere(uint32_t slices, uint32_t rings, float radius, const Vector3& pos, const Color& color)
{
	Vector3 v0 = Vector3::Zero;
	Vector3 v1 = Vector3::Zero;
