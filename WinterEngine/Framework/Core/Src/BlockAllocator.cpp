...
BlockAllocator::~BlockAllocator()
{
	...
	std::free(mData);
	mData = nullptr;
	LOG("%s destructed, Allocated: %zu, Free: %zu, Highest: %zu", mName.c_str(), mBlocksAllocatedCurrent, mBlocksFreed, mBlocksHighest);
}
...
void* BlockAllocator::Allocate()
{
	...
	LOG("%s allocated blocks at %p, Allocated: %zu, Highest: %zu", mName.c_str(), freeBlock, mBlocksAllocatedCurrent, mBlocksHighest);
