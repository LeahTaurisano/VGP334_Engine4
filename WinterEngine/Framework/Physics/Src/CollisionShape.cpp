...
void CollisionShape::InitializeBox(const WinterEngine::Math::Vector3& halfExtents)
{
	ASSERT(mCollisionShape == nullptr, "CollisionShape: shape is already initialized");
	mCollisionShape = new btBoxShape(TobtVector3(halfExtents));
}

void CollisionShape::InitializeHull(const WinterEngine::Math::Vector3& halfExtents, const WinterEngine::Math::Vector3& origin)
{
	ASSERT(mCollisionShape == nullptr, "CollisionShape: shape is already initialized");
	btConvexHullShape* hullShape = new btConvexHullShape();
	std::vector<WinterEngine::Math::Vector3> points =
	{
	...
	for (WinterEngine::Math::Vector3& point : points)
	{
